...
exports.execute = function(config, callback) {
    ...
    return async.waterfall([
        ...
        function(callback) {
            var sheet = sheetFront.replace('<!-- colsdef -->', config.colsDef || '');
            return write(sheet, callback);
        },
        function(callback) {
                    ...
                    for (j=0; j < colsLength; j++) {
                        ...
                        switch (cellType) {
                            ...
                            case 'bool':
                                ...
                                break;
                            case 'formula':
                                row += addFormulaCol(getColumnLetter(j+1)+currRow, cellData, styleIndex);
                                break;
                            default:
...
var addFormulaCol = function(cellRef, value, styleIndex){
    styleIndex = styleIndex || 0;
    if (value===null) {
        return "";
    } else {
        return '<x:c r="'+cellRef+'" s="'+ styleIndex +'" t="n">' + '<x:f>' + value + '</x:f>' + '<x:v>'+value+'</x:v></x:c>';
    }
}
